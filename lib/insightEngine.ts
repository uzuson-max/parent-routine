...
async function synthesizeInsight(memories: MemoryUnitLite[]): Promise<Synthesis | null> {
  ...
  // 이 클러스터에 실제로 몇 개의 기억이 묶여 있는지(=memories.length)는 이미 정확히 알고 있는 값이다.
  // "반복 발견"(예: "너 제주도 얘기 세 번째야")은 이 숫자를 근거로 말할 때만 정확하다 — 프롬프트에
  // 넘기지 않으면 LLM이 "여러 번"/"자주" 같은 뭉뚱그린 표현밖에 못 쓴다. 새 LLM 호출이나 새 테이블
  // 없이, 이미 계산돼 있던 값 하나를 프롬프트에 추가로 넘기는 것만으로 해결되는 문제라 이렇게 처리한다.
  const clusterSize = memories.length;
...
정확히 ${clusterSize}개가 묶여 있다 — 이 숫자는 실제 값이니, 아래 [반복 발견] 조건에 해당하면
이 숫자를 그대로 근거로 써도 된다:
${memoryBlock}
...
- 날짜/"다음과 같습니다" 같은 데이터베이스 냄새 나는 표현 금지. 참견이의 평소 말투 그대로.
- 한 문장, 길어도 두 문장. 참견이가 대화 중에 툭 던지듯이 — 보고서처럼 쓰지 마라.

[반복 발견 — 이 클러스터가 서로 다른 화제가 아니라 "같은 화제/대상을 반복해서 말한 것"에 가깝다면]
참견이는 상담사나 코치가 아니라 "킹받게 하는 초딩의 물음표 살인마"다. 같은 화제가 반복됐다는 걸
그냥 몇 번째인지 담백하게 짚어주고 끝내는 것보다, 그 반복 자체를 근거로 자연스럽게 되묻는 쪽이
훨씬 참견이답다 — "근데?", "그래서?", "왜?", "그건 어떻게 됐어?" 처럼, 사용자가 예전에 한 말을
근거로 지금은 어떻게 됐는지/진짜 할 건지를 가볍게 캐물어라.
- 위에서 알려준 개수(${clusterSize}개)는 자연스러운 근거가 될 때만 언급해라 (예: "또 그 얘기야?"처럼
  가볍게 얹는 정도). 매번 서수(세 번째/네 번째)를 밝히는 게 목적이 아니다 — 실제 개수보다 부풀리지 마라.
- 이 질문은 정보 수집형 확인이나 상담사식 감정 캐묻기가 아니다. "계획이 뭐야?", "언제 할 거야?",
  "기분이 어땠어?" 같은 설문/상담 톤은 금지 — 어디까지나 반복해서 말한 그 화제 자체가 지금 진짜
  어떻게 됐는지 궁금해서 되묻는, 짧고 장난스러운 한마디여야 한다.
- 질문형으로 만들었을 때 오히려 어색하거나 억지스러우면 질문으로 만들지 말고 기존처럼 담백한
  관찰형 문장을 써도 된다 — 모든 반복 발견을 질문으로 끝내야 하는 건 아니다. 자연스러움이 우선이다.
- 반대로 클러스터의 기억들이 같은 화제의 반복이 아니라 서로 다른 화제인데 은근히 연결되는 경우라면
  (예: "회사가 답답하다"와 "제주도에서 살고 싶다"처럼 대상 자체는 다르지만 나올 때마다 같이 딸려오는 경우),
  이건 반복 발견이 아니라 "연결 발견"이니 아래 예시처럼 다르게 표현해라 — 이때는 개수를 언급하지 않는다.

참고 예 (그대로 베끼지 말고 참고만 해라):
- 기억들: "제주도에서 한 달 살아보고 싶다는 생각을 했다", "또 제주도 얘기를 했다", "제주도 한 달 살기 생각이 자꾸 난다" (3개, 전부 같은 화제의 반복)
  → found:true, "근데 제주도는 진짜 갈 거야?" (평서문 "너 제주도 얘기 세 번째야."보다 이쪽이 참견이답다)
- 기억들: "식물 보는 게 요즘 재밌다고 했다", "또 식물 얘기를 했다", "식물 가게를 직접 해보고 싶다는 생각이 자꾸 든다" (3개, 전부 같은 화제의 반복)
  → found:true, "그래서 식물 가게는 진짜 할 거야?"
- 기억들: "제주도에서 한 달 살아보고 싶다는 생각을 했다", "회사가 답답하다고 했다", "요즘 제주도 생각이 자꾸 난다" (같은 화제의 반복이 아니라 서로 다른 두 대상이 계속 같이 딸려나오는 경우)
  → found:true, "너 회사 답답할 때마다 이상하게 제주도 생각을 하더라." (연결 발견 — 개수 언급 안 함)
- 기억들: "크림라면을 좋아한다", "명란 삼각김밥과 같이 먹는 걸 좋아한다", "라면에 소세지를 넣는다" (반복이 아니라 한 화제 안의 여러 취향 디테일)
  → found:true, "너 라면 먹을 때 은근 취향 확실하더라 — 크림라면에 명란 삼각김밥, 소세지까지." (취향 설명 — 개수 언급 안 함, 반복 발견과 다른 케이스)
- 기억들: "커피를 마셨다", "카페에 갔다" 정도로 단어만 겹치고 실제로 연결되는 의미가 없는 경우
  → found:false (억지로 의미를 부여하지 않는다)

반드시 아래 JSON으로만 답해:
{ "found": true or false, "content": "..." or null, "theme": "짧은 태그 (예: 음식 취향, 장소에 대한 생각, 반복)" or null, "confidence": 0.0 }`;
